...
#[tauri::command]
async fn blank_animation<R: Runtime>(
...
#[tauri::command]
async fn save_animation<R: Runtime>(
    _app: tauri::AppHandle<R>,
    _window: tauri::Window<R>,
...
#[tauri::command]
async fn delete_animation<R: Runtime>(
    _app: tauri::AppHandle<R>,
    _window: tauri::Window<R>,
    id: Uuid,
) -> Result<define::Animation, String> {
    data::DATA
        .lock()
        .unwrap()
        .remove_animation(&id)
...
fn open_stage_window<R: Runtime>(app: tauri::AppHandle<R>, label: &String, name: &String) -> () {
    // 1024×768
    let window =
        tauri::WindowBuilder::new(&app, label, tauri::WindowUrl::App("./stage.html".into()))
...
#[tauri::command]
async fn get_stage_by_id<R: Runtime>(
    _app: tauri::AppHandle<R>,
    _window: tauri::Window<R>,
    id: Uuid,
) -> Result<define::Stage, String> {
    data::DATA
        .lock()
        .unwrap()
        .get_stage(&id)
...
async fn stage_creator<R: Runtime>(app: tauri::AppHandle<R>, id: Option<Uuid>) {
    ...
    if let Some(window) = app.get_window(&label) {
        ...
        window
            .close()
            .expect(format!("Stage editor window {} does not react", label).as_str());
    }
    let name = data::DATA
        .lock()
        .unwrap()
...
#[tauri::command]
async fn delete_stage<R: Runtime>(
    _app: tauri::AppHandle<R>,
    window: tauri::Window<R>,
    id: Uuid,
) -> Result<bool, String> {
    ...
    if count > 0 {
        let res = tauri::api::dialog::blocking::confirm(
            ...
            "Delete Stage",
            format!(
                "This stage is referenced by {} scenes. Are you sure you want to delete it?",
                count
            ),
        );
        if !res {
